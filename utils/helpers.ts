...
export function formatDate(dateInput: string | Date): string {
  ...
  return `${day}/${month} (${dayOfWeek} - ${formattedHours}:${minutes} ${ampm})`
}
...
export const groupTransactionsByBank = (transactions: TransformedTransaction[]) => {
  const groupedTransactions = transactions.reduce((acc, transaction) => {
    ...
    const { bank_id, bank_name, logo_url } = bank;
    ...
    acc[bank_id].transactions.push(transaction);

    if (transaction_type === "Expense") {
      acc[bank_id].expenseAmount += amount;
    } else if (transaction_type === "Income") {
      acc[bank_id].incomeAmount += amount;
    }
